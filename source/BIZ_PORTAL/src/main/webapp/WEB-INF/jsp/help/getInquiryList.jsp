...
<body>
 ...
 <c:forEach var="result" items="${inquiryList}" varStatus="status">   
   <tr align="center" bgColor="#cccccc">
	   ...
	   <td width="35%" class="td"><c:out value="${result.QA_TYPE}"/></td>
	   <td width="15" class="td"><a href="${HOME}/help/getNoticeDetail.do?ctnId=${result.QST}"><c:out value="${result.QST}"/></a></td>
	   <td width="25%" class="td">	
	   							<fmt:formatDate value="${result.REG_DT}"  pattern="yyyy-MM-dd"/>
	   </td>
	   <td width="15%" class="td"><c:out value="${result.ANS_YN}"/></td>
   </tr>
 </c:forEach>    
	<tr>
		<td>
